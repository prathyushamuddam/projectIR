...
.controller('gsr', function ($scope, $cookieStore, $location, $filter, http) {
    ...
    if (! $cookieStore.get('email')) {
        ...
        $scope.examineLife = function (obj, index) {
            ...
            var getGSRImagesParams = {
                GSREventId: present.id
            }
            http.async('/getGSRImages', getGSRImagesParams).then(function (data) {

                for (var i = 0; i < data.length; i++) {
                    $scope.examine[i] = {
                        path: data[i].path,
                        date: minsToExamine[i].date,
                        gsr: minsToExamine[i].gsr,
        ...
        // function to generate gsr events
        $scope.generateGSREvents = function () {

            // assign each gsr reading a color code
            var colors = {
                0: '#ffffff', // lowest gsr reading
                1: '#c2e2b2',
                2: '#abd79c',
                3: '#83cb80',
                4: '#6ebb69',
            ...
            for (var i = 0; i < 10; i++) {
                ...
                for (var j = 0; j < $scope.lifelogData.length; j++) {
                    ...
                    //////////////////////////////////////////////////////////// TESTING
                    if ($scope.lifelogData[j].gsr > 1) {
                        console.log($scope.lifelogData[j].gsr);
                    }

                    // assign color to specific gsr code
                    if ($scope.lifelogData[j].gsr > prev && $scope.lifelogData[j].gsr <= next) {
                        $scope.lifelogData[j].color = colors[i];
                    }

                    // if last division, make everything above it final color code
                    if (i == 9) {
                        if ($scope.lifelogData[j].gsr > next) {
                            $scope.lifelogData[j].color = colors[i];
                        }
                    }
        ...
        $scope.generateNewLifelog = function () {
            ...
            http.async('/getGSREvents ', getGSREventsParameters).then(function (data) {

                $scope.lifelogData = data;
                $scope.generateGSREvents();
        ...
        http.async('/Login', loginParameters).then(function (data) {
            ...
            http.async('/getGSRDates ', getGSRDatesParameters).then(function (data) {
